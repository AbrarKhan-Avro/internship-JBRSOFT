...
function App() {
  ...
  // Change this slug to any page you created in Django admin (like "registration")
  const pageSlug = "registration";

  useEffect(() => {
    axios.get(`${API_BASE}/pages/${pageSlug}/`)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post(`${API_BASE}/submit/${pageSlug}/`, formData)
  ...
  if (!page) return <div className="p-8 text-center text-gray-500">Loading form...</div>;
  ...
  if (submitted) return (
    <div className="h-screen flex flex-col items-center justify-center bg-green-100 text-green-800">
      ...
      <button onClick={() => setSubmitted(false)} className="mt-4 px-4 py-2 bg-green-700 text-white rounded">
        Submit Another
      </button>
    </div>
  );
      ...
      >
        ...
        {page.fields.map((field) => (
          <div key={field.id} className="mb-4">
            ...
            {["text", "email", "password", "number", "date"].includes(field.field_type) && (
              <input
